...
from app.users.helpers import send_new_sms_code
...
async def register_user(reg_user: SRegisterUser):
...
@router.post(
    "/sms_verification"
)  # Переписать!!! Разделить на функции, добавить else latest_sms_code
async def sms_verification(user_phone: SUsersPhone):
    ...
    if latest_smscode:
        ...
        expires_at_utc = latest_smscode.expires_at.replace(
            tzinfo=timezone.utc
...
async def login(response: Response, user: SUserAuth = Depends(validate_auth_user)):
...
def auth_refresh_jwt(response: Response, user: SUsers = Depends(get_current_auth_user_refresh)):
    access_token = create_access_token(user)
    auth_user_set_cookie(response, ACCESS_TOKEN_TYPE, access_token)
