...
from app.exceptions import (
    IncorrectSmsValidationException,
    IncorrectTokenFormatException,
    InvalidToken,
    SmsValidationExpired,
...
from app.users.token import ACCESS_TOKEN_TYPE, REFRESH_TOKEN_TYPE, TOKEN_TYPE_FIELD
...
async def validate_auth_user(
    ...
    expires_at_utc = sms_code.expires_at.replace(
        tzinfo=timezone.utc
    )  # Добавляем временную зону к полученному времени
...
def get_auth_user_from_token_of_type(token_type: str):
    async def get_auth_user_from_token(
        ...
        payload: dict = Depends(
            get_current_token_payload_with_type(token_type)
        ),  # Зависимость для токена
    ) -> SUsers:
...
def get_current_active_auth_user(user: SUsers = Depends(get_current_auth_user)):
    if user.is_active:
        return user
    raise UserInActive
...
def get_admin_active_auth_user(user: SUsers = Depends(get_current_active_auth_user)):
