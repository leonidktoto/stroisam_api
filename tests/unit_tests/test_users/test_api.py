...
async def test_login(username, password, status_code, ac: AsyncClient):
    ...
    if status_code == 200:
        ...
        assert "access" not in response.cookies, "Cookie 'access' не должна быть установлена при ошибке"
        assert "refresh" not in response.cookies, "Cookie 'refresh' не должна быть установлена при ошибке"

@pytest.mark.parametrize("phone, status_code", [
    ("9170170001", 409), 
  #  ("9170176646", 200), 
    ("9170176955", 423), 
    ("1", 422), 
...
async def test_sms_verification(phone, status_code, ac: AsyncClient):
    response = await ac.post("/users/sms_verification", json={
        "phone": phone,
    }, follow_redirects=True)
