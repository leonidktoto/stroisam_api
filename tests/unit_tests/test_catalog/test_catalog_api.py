...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            26,
            200,
            [{"id": 15, "article": None, "product_name": "Доска", "price": 13, "image_url": None}],
        ),
        (999, 200, []),
        ("9zxcc", 422, None),
        ("??", 404, None),
...
async def test_get_products_by_category_id(
    category_id, status_code, expected_response, ac: AsyncClient
):
...
@pytest.mark.parametrize(
    ...
    [
        (
            24,
            [
                {"attribute_name": "Материал", "attribute_value": ["Сосна", "Береза", "Хвоя"]},
                {"attribute_name": "Сорт", "attribute_value": ["Люкс", "Оптима"]},
                {"attribute_name": "Размер", "attribute_value": ["40x50x2000 мм", "20x40x2000 мм"]},
            ],
            200,
            [
                {
...
@pytest.mark.asyncio
async def test_get_product_by_filter(
    category_id, filters, status_code, expected_response, ac: AsyncClient
):
    response = await ac.post(
        f"/products/filter?category_id={category_id}", json=filters, follow_redirects=True
    )
    assert response.status_code == status_code
